...
export async function sha256Hex(body: ArrayBuffer | Uint8Array | string): Promise<string> {
  const digest = await getCrypto().subtle.digest("SHA-256", toArrayBuffer(body));
...
export async function verifyPassword(password: string, encodedHash: string): Promise<boolean> {
  const [scheme, iterationsValue, saltHex, expectedHex] = encodedHash.split("$");
  if (scheme !== "pbkdf2_sha256" || !iterationsValue || !saltHex || !expectedHex) {
    return false;
  }
