...
export function createNodeRuntime(): NodeRuntime {
  ...
  const localStorageDir =
    process.env.LOCAL_STORAGE_DIR ?? "/data/pagevault/objects";
  ...
  return {
    ...
    migrate: async () => {
      await repository.migrate(
        resolve(process.cwd(), "migrations/0001_initial.sql"),
      );
      await repository.migrate(
...
export function createStaticAssetFetcher(
  root = resolve(process.cwd(), "apps/admin/dist"),
) {
  return async function fetchAsset(request: Request): Promise<Response> {
    ...
    const target = candidate.startsWith(root)
      ? candidate
