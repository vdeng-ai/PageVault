...
export function registerAuthRoutes(app: Hono<HonoRuntime>): void {
  app.post("/api/auth/login", async (c) => {
    ...
    const validPassword = await verifyPassword(parsed.data.password, c.env.ADMIN_PASSWORD_HASH);
    if (!validEmail || !validPassword) {
      return c.json({ error: "Invalid credentials" }, 401);
    }

    const { value } = await createSession(parsed.data.email, c.env.SESSION_SECRET);
  ...
  app.post("/api/auth/logout", async (c) => {
    ...
    if (session && !(await verifyCsrfToken(session, c.req.header("X-CSRF-Token") ?? null))) {
