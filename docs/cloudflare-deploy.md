...
5. `pnpm wrangler d1 migrations apply htmlbed-db --remote`
6. `pnpm tsx scripts/hash-password.ts`
7. `pnpm wrangler secret put ADMIN_EMAIL`
8. `pnpm wrangler secret put ADMIN_PASSWORD_HASH`
9. `pnpm wrangler secret put SESSION_SECRET`
10. Configure routes or custom domains for `admin-html.example.com/*` and `h.example.com/*`.
11. `pnpm run build`
12. `pnpm wrangler deploy`
...
Worker secrets are initialized with `pnpm wrangler secret put` and remain managed by Cloudflare.
